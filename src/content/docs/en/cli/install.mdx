...
<Aside type="note" title="yarn usually doesn't need anything special">
Classic yarn (v1) runs a dependency's `postinstall` script automatically — no approval step needed. If the project uses Yarn Berry (v2+, especially with PnP mode), lifecycle script execution can behave differently; check the project's own yarn configuration.
</Aside>
