...
Both tools (`shane-new-post` for blogs, `shane-new-doc` for docs sites) install the same way. The examples below use `shane-new-post` — swap in `shane-new-doc` for the other tool.
...
`pnpm new-post` / `pnpm new-doc` is ready to use immediately.
...
With npm an extra `--` is needed before the flag so it gets passed through to the installer. pnpm doesn't need it.
</Aside>
...
pnpm v9 / v10 blocks a new dependency's install scripts by default. Run `pnpm approve-builds`, use the arrow keys to select `shane-new-post`, press space to check it, then enter to confirm. To skip this every time, add to the project's `package.json`:
...
That performs the exact same steps postinstall always performs — nothing different, just triggered manually instead of automatically. If for some reason that file also doesn't exist (a very unusual install failure), add the line to `package.json` yourself. Before:
