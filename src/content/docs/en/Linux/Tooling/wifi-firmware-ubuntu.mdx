...
<Steps>
- Check rfkill: `rfkill list all` — unblock with `sudo rfkill unblock all`.
- Kernel messages: `dmesg | grep -i firmware`.
- Interface state: `ip link`, `iw dev`, `nmcli device status`.
- Capture logs: `journalctl -u NetworkManager -n 200`.
- Try a live kernel (booted from live USB) to exclude current installation issues.
- If module mismatch after kernel update, ensure `linux-headers-$(uname -r)` is installed and re-run DKMS builds.
</Steps>
