...
pnpm v9 / v10 默认会拦截新依赖的安装脚本（`postinstall`）。执行 `pnpm approve-builds`，方向键选中对应的包名（`shane-new-post` 或 `shane-new-doc`），按空格勾选，回车确认即可。想以后都不用手动批准，可以在项目的 `package.json` 里加：
...
<Aside type="note" title="yarn 一般不用管这个">
经典 yarn（v1）安装时会自动执行依赖的 `postinstall` 脚本，不需要额外批准。如果项目用的是 Yarn Berry（v2 及以上，尤其开了 PnP 模式），生命周期脚本的执行方式可能不一样，具体以项目自己的 yarn 配置为准。
</Aside>
