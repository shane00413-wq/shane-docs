...
两个工具（`shane-new-post` 给博客用，`shane-new-doc` 给文档站用）安装方式完全一样，下面统一用 `shane-new-post` 举例，把命令里的名字换成 `shane-new-doc` 就是另一个工具的用法。
...
`pnpm new-post` / `pnpm new-doc` 立刻就能用。
...
npm 需要在参数前面加一个额外的 `--` 才能把 `--lang` 透传给安装器，pnpm 不需要。
</Aside>
...
大部分情况下这两样都会自动到位，可以直接跳到 [shane-new-post 使用指南](/zh-cn/cli/new-post/) 或 [shane-new-doc 使用指南](/zh-cn/cli/new-doc/)。但有些包管理器——尤其是较新版本的 pnpm——默认会跳过生命周期脚本（`postinstall`），一些 CI / 非交互式安装环境也可能全局开了 `--ignore-scripts`。这种情况下依赖会正常写进 `package.json`，但 `scripts/` 下什么都不会生成，`scripts` 字段那条也不会被写进去。一键安装器（`npm create` / `pnpm create`）已经内置了针对这种情况的二次检测和自动补跑——具体怎么做的见[安装原理详解](/zh-cn/cli/internals/)。**手动安装**（上面的方式二）没有这层保险，所以最容易碰到这种情况的就是它。
